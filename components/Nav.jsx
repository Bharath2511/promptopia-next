...
const Nav = () => {
  ...
  return (
       ...
       <Link href='/' className='flex gap-2 flex-center'>
        ...
        <div className="sm:flex hidden">
            {session?.user ? (<div className='flex gap-3 md:gap-5'>
                ...
                <Link href="/profile"><Image src={session.user.image} width={37} height={37} onClick={()=>{}} className='rounded-fill' alt='profile' /></Link>
            </div>):(<>
                {providers && 
    ...
    <div className='sm:hidden flex relative'>
        {session?.user ? (
            <div className='flex'>
                <Image src={session.user.image} width={37} height={37} onClick={()=>{setToggleDropdown(prev=>!prev)}} className='rounded-fill' alt='profile' />
                {toggleDropdown && (
